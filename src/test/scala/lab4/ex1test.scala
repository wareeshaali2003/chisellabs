...
val result2_ : BigInt = if (result1  < 0)
    ( BigInt (0xFFFFFFFFL ) + result1 + 1) & 0xFFFFFFFFL
    else result1 & 0xFFFFFFFFL
c.io.in_A.poke(src_a.U)
c.io.in_B.poke(src_b.U )
c.io.alu_Op.poke ( aluop )
c.clock.step (1)
// c.io.out.expect( result2_.asUInt )
}
c.clock.step (100)
}
}
